...
function init(callback){
    async.series([
      ...
      (cb) => {
        blockstatic.buildPages('./src/content/case_studies', './dist/case-studies', casestudyOptions, cb);
      },
      (cb) => {
        blockstatic.jsonFeedPages('./src/content/articles', '/articles', articleOptions.site, (err, jsonfeed) => {
            if(err){
              return cb(err);
            }
      ...
      (cb) => {
        blockstatic.rssFeedPages('./src/content/articles', '/articles', articleOptions.rss, (err, rssfeed) => {
            if(err){
              return cb(err);
            }
      ...
      (cb) => {
        blockstatic.indexPages('./src/content/articles', '/articles', (err, jsonindex) => {
            if(err){
              return cb(err);
            }
